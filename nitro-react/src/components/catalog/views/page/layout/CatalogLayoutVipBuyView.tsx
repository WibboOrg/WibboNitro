...
import { AutoGrid, Button, Column, Flex, Grid, LayoutBadgeImageView, LayoutCurrencyIcon, LayoutGridItem, LayoutLoadingSpinnerView, Text } from '../../../../../common';
...
export const CatalogLayoutVipBuyView: FC<CatalogLayoutProps> = props =>
{
    ...
    {
        if (!purse.isVip) return currentPage.localization.getText(0);
                    ...
                    { clubOffers && (clubOffers.length > 0) && clubOffers.map((offer, index) =>
                    {
                        return (
                            <LayoutGridItem key={ index } column={ false } center={ false } alignItems="center" justifyContent="between" itemActive={ pendingOffer === offer } className="p-1" onClick={ () => setOffer(offer) }>
                                ...
                                <LayoutBadgeImageView badgeCode={ offer.productCode === 'premium_club_3' ? 'WC_LEGEND' : offer.productCode === 'premium_club_2' ? 'WC_EPIC' : 'WC_CLASSIC' } />
                                <Column justifyContent="end" gap={ 0 }>
                                    <Text textEnd>{ offer.productCode === 'premium_club_3' ? 'Premium légende' : offer.productCode === 'premium_club_2' ? 'Premium épique' : 'Premium classique' }</Text>
