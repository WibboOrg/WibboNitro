...
import { Base, Flex, FriendlyTimeView, LayoutAvatarImageView, Text } from '../../../../../common';
...
export const FriendsMessengerThreadGroup: FC<{ thread: MessengerThread, group: MessengerThreadChatGroup }> = props =>
{
    ...
    const chatTime = useMemo(() => 
    {
        ...
        return lastChat ? (Math.floor(new Date().getTime() - lastChat.date.getTime()) / 1000) + lastChat.secondsSinceSent : 0;
    }, [ group ]);
    ...
    return (
        <Flex fullWidth justifyContent={ isOwnChat ? 'end' : 'start' } gap={ 2 }>
            ...
            <Base className="mb-2">
                <Base className={ 'bg-light text-black border-radius rounded py-1 px-2 messages-group-' + (isOwnChat ? 'right' : 'left') }>
                ...
                <Text small italics textEnd={ isOwnChat }><FriendlyTimeView seconds={ chatTime } /></Text>
            </Base>
            { isOwnChat &&
                <Base shrink className="message-avatar">
