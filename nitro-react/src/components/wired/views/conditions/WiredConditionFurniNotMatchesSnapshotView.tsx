...
export const WiredConditionFurniNotMatchesSnapshotView: FC<{}> = props =>
{
    ...
    return (
        <WiredConditionBaseView requiresFurni={ WiredFurniType.STUFF_SELECTION_OPTION_BY_ID } hasSpecialInput={ true } save={ save }>
            ...
            <Column gap={ 1 }>
                <Text bold>{ LocalizeText('wiredfurni.params.not_requireall.furni.match') }</Text>
                { [ 0, 1 ].map(value =>
                {
                    return (
                        <Flex alignItems="center" gap={ 1 } key={ value }>
                            ...
                            <Text>{ LocalizeText('wiredfurni.params.not_requireall.furni.match.' + value) }</Text>
                        </Flex>
                    )
                }) }
            </Column>
