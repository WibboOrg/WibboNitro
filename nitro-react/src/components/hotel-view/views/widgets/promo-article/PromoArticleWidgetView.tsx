...
import { GetConfiguration, LocalizeText, OpenUrl, SendMessageComposer } from '../../../../../api';
        ...
        <div className="promo-articles widget mb-2">
    		...
    		{ articles && articles[index] &&
				<div className="promo-article d-flex flex-row row mx-0">
			  		<div className="promo-article-image" style={ { backgroundImage: `url(${ GetConfiguration<string>('image.library.url') + articles[index].imageUrl })` } }/>
				    <div className="col-6 d-flex flex-column h-100">
				        <h3 className="my-0">{ articles[index].title }</h3>
				        <b>{ articles[index].bodyText }</b>
				        <button className="btn btn-sm mt-2 btn-gainsboro" onClick={ event => OpenUrl(articles[index].linkContent) }>{ articles[index].buttonText }</button>
				    </div>
          		</div>
       		}
    	</div>
