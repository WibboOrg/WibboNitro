            ...
            items.push(
                <LayoutGridItem key={ index } position="relative" overflow="hidden" className="nitro-avatar-editor-wardrobe-figure-preview">
                    ...
                    <Flex gap={ 1 } className="button-container">
                        <Button variant="link" fullWidth onClick={ event => saveFigureAtWardrobeIndex(index) }><FaSave className="fa-icon w-12 h-12" /></Button>
                        { figureContainer &&
                        <Button variant="link" fullWidth onClick={ event => wearFigureAtIndex(index) } disabled={ (clubLevel > GetClubMemberLevel()) }><FaSync className="fa-icon w-12 h-12" /></Button> }
                    </Flex>
                </LayoutGridItem>
            );
        });
