...
export class SoundManager extends NitroManager implements ISoundManager
{
    ...
    private onEvent(event: INitroEvent)
    {
        switch(event.type)
        {
            ...
            case NitroSettingsEvent.SETTINGS_UPDATED: {
                ...
                const volumeSystemUpdated = castedEvent.volumeSystem !== this._volumeSystem;
                ...
                if(volumeSystemUpdated) this.updateInternalSamplesVolume(this._volumeSystem);
    ...
    private playSample(sample: HTMLAudioElement, volume: number, loop: boolean = false): void
    {
    ...
    private playExternalSample(code: string, type: number, loop: boolean): void
    {
        let sample = this._internalSamples.getValue(code);

        if(!sample)
        {
            const sampleUrl = Nitro.instance.getConfiguration<string>('cdn.sounds.url');

            sample = new Audio(sampleUrl.replace('%sample%', code));
            this._internalSamples.add(code, sample);
        }

        const volume = this._volumeSystem;

        this.playSample(sample, volume, loop);
    }
    ...
    private playFurniSample(objectId: number, code: number, pitch: number): void
    {
        ...
        this.playSample(sample, this._volumeFurni);
    }
