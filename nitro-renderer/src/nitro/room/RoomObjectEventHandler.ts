...
export class RoomObjectEventHandler extends Disposable implements IRoomCanvasMouseListener, IRoomObjectEventManager
{
    ...
    public handleRoomObjectEvent(event: RoomObjectEvent, roomId: number): void
    {
        ...
        switch(event.type)
        {
            ...
            case RoomObjectWidgetRequestEvent.HIDE_TWITCH_STREAM_DISPLAY:
    ...
    private onRoomObjectWidgetRequestEvent(event: RoomObjectWidgetRequestEvent, roomId: number): void
    {
        ...
        switch(event.type)
        {
            ...
            case RoomObjectWidgetRequestEvent.HIDE_HIGH_SCORE_DISPLAY:
                ...
                return;
            case RoomObjectWidgetRequestEvent.TWITCH_STREAM_DISPLAY:
                eventDispatcher.dispatchEvent(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_TWITCH_STREAM_DISPLAY, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.HIDE_TWITCH_STREAM_DISPLAY:
                eventDispatcher.dispatchEvent(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_HIDE_TWITCH_STREAM_DISPLAY, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.INERNAL_LINK:
    ...
    public setSelectedAvatar(k: number, _arg_2: number, _arg_3: boolean): void
    {
        ...
        if(_arg_3)
        {
            ...
            if(_local_5 && _local_5.logic)
            {
                ...
                if(location) this._roomEngine.connection.send(new RoomUnitLookComposer(~~(location.x), ~~(location.y), _arg_2));
            }
        }
